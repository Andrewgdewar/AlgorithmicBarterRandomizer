...
export class globalValues {
  ...
  public static updateBarters(time?: number) {
    ...
    const filterLootList = [...lootList].filter(
      (id) => !checkParentRecursive(id, items, excludableParents)
    );
    ...
    const getAlt = (
      ...
      const filteredLootList = cutLootList.filter(
        (id) => !itemSet.has(id) && !excludedItemsList.has(id)
      );
    ...
    this.Logger.info(
      `AlgorithmicBarterRandomizer: Updated ${
